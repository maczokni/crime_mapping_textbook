...
This week, we're going to leave behind the neat and easy-to-access data typical of 'western' countries. Instead, we're going to look at some data from India! We'll be using two datasets:
...
So, before continuing you should look at the table and graph you've just made and decide which shapefile we should use now.
...
district_yields <- shapefile %>%
  ...
  select(YEAR, NAME, STATE_UT, RICE_TA, RICE_TQ) %>% # 'select' only selects the columns we're interested in
...
We can see that the district with the highest rice production rate in 1971 had a rate of 48,309 tonnes of rice per 100,000 individuals. That is very high. Just to put it into context in the UK is about 0.92. Where is that place? I can tell you is a place call Borden. Check it out:

```{r}
# This will show us the row with the highest yield
print(district_yields[which.max(district_yields$rice_yield),])

# Subset the 'Saharsa' district data
saharsa <- subset(district_yields, NAME == "saharsa")

```

[Saharsa district](https://en.wikipedia.org/wiki/Saharsa_district) in Bihar, India is known for high quality corn, much of which is exported here to the UK! But Saharsa isn't particularly known for its great rice production. So, what’s going on here? Why do we have a high production rate (yield) in Saharsa that makes it look like the rice capital of India?
...
```{r}
print(filter(district_yields, district == 'saharsa', YEAR == '1971')$RICE_TA)

area_comparison <- district_yields %>%
  group_by(district) %>%
  summarise(mean_area = mean(RICE_TA)) %>%
  select(district, mean_area)
