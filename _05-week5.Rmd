...
ggplot(district_match_results) +
  geom_line(aes(x = shapefile_year, y = district_match_percent, group = 1))
...
# Filter the yield data to only include 1971 info
yield_1971 <- filter(yield_data, YEAR == '1971')
...
yield_1971$DISTNAME <- make_clean_names(yield_1971$DISTNAME)
yield_1971$STNAME <- tolower(yield_1971$STNAME)
...
district_yields <- shapefile %>%
  right_join(yield_1971, by = c('NAME' = 'DISTNAME', 'STATE_UT' = 'STNAME')) %>%
  select(NAME, STATE_UT, TOT_POP, RICE_TQ) %>% # 'select' only selects the columns we're interested in
  mutate(rice_per_100k = RICE_TQ*1000/TOT_POP*100000) %>%
